...
let Remove = (req, res, id) => {
  return new Promise(async (resolve, reject) => {
    try {
      const DeleteData = await db.Inventory_Detail.destroy({
        where: {
          id: id,
        },
      });
...
let Create = (req, res, dataAdd) => {
  return new Promise(async (resolve, reject) => {
    try {
      const PostData = await db.Inventory_Detail.create({
        inventory_count_id: dataAdd.inventory_count_id,
        product_id: dataAdd.product_id,
        qty: dataAdd.qty,
        price: dataAdd.price,
        sale_price: dataAdd.sale_price,
        type: dataAdd.type,
        EndingStocks: dataAdd.EndingStocks
      });
